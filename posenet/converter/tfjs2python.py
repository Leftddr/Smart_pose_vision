...
def load_variables(chkpoint, base_dir=BASE_DIR):
    ...
    for x in variables:
        ...
        d = tf.compat.v1.reshape(d, variables[x]["shape"])
        variables[x]["x"] = tf.compat.v1.Variable(d, name=x)
...
def build_network(image, layers, variables):
    ...
    def _conv_to_output(mobile_net_output, output_layer_name):
        w = tf.compat.v1.nn.conv2d(mobile_net_output, _weights(output_layer_name), [1, 1, 1, 1], padding='SAME')
        w = tf.compat.v1.nn.bias_add(w, _biases(output_layer_name), name=output_layer_name)
        return w

    def _conv(inputs, stride, block_id):
        return tf.compat.v1.nn.relu6(
            tf.compat.v1.nn.conv2d(inputs, _weights("Conv2d_" + str(block_id)), stride, padding='SAME')
    ...
    def _separable_conv(inputs, stride, block_id, dilations):
        ...
        w = tf.compat.v1.nn.depthwise_conv2d(
            inputs, _depthwise_weights(dw_layer), stride, 'SAME', rate=dilations, data_format='NHWC')
        w = tf.compat.v1.nn.bias_add(w, _biases(dw_layer))
        w = tf.compat.v1.nn.relu6(w)

        w = tf.compat.v1.nn.conv2d(w, _weights(pw_layer), [1, 1, 1, 1], padding='SAME')
        w = tf.compat.v1.nn.bias_add(w, _biases(pw_layer))
        w = tf.compat.v1.nn.relu6(w)
    ...
    heatmaps = tf.compat.v1.sigmoid(heatmaps, 'heatmap')
...
def convert(model_id, model_dir, check=False):
    ...
    with cg.as_default():
        ...
        with tf.compat.v1.Session() as sess:
            ...
            image_ph = tf.compat.v1.placeholder(tf.compat.v1.float32, shape=[1, None, None, 3], name='image')
